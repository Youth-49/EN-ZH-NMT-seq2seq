...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        pack = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (enc_last_hidden, enc_last_cell) = self.encoder(pack)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, padding_value=self.vocab.src["<pad>"])
        enc_hiddens = enc_hiddens.permute(1,0,2)
        init_decoder_hidden = self.h_projection(torch.cat((enc_last_hidden[0], enc_last_hidden[1]), 1))
        init_decoder_cell = self.c_projection(torch.cat((enc_last_cell[0], enc_last_cell[1]), 1))
